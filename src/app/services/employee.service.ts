...
export class EmployeeService {
  ...
  getEmployee(id: string): Observable<Employee> {
    ...
    return this.http.get<Employee>(url).pipe(
      tap(_ => this.messageService.add(`EmployeeService: fetched employee id=${id}`)),
  ...
  updateEmployee(updatedEmployee: Employee): Observable<any> {
    ...
    return this.http.put(url, updatedEmployee, this.httpOptions).pipe(
      tap(_ => this.messageService.add(`EmployeeService: updated employee id=${updatedEmployee.id}`)),
  ...
  deleteEmployee(id: string | undefined): Observable<Employee> {
    ...
    return this.http.delete<Employee>(url, this.httpOptions).pipe(
      tap(_ => this.messageService.add(`EmployeeService: deleted employee id=${id}`)),
  ...
  private generateNewId(): Observable<string> {
    return this.http.get<Employee[]>(this.employeesUrl).pipe(
      map(employees => {
        const maxIdNumber = employees.reduce((max, emp) =>
          (Number(emp.id) > max ? Number(emp.id) : max), 0);
